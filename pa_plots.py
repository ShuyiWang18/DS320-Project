...
def train_lr_rf_for_scheme(scheme_name, full, train, valid, test):
...
def plot_roc_pr_hmda(probs, y_test):
    ...
    plt.legend()
    plt.tight_layout()
    out_path = os.path.join(REPORT_DIR, "roc_hmda_lr_rf.png")
    ...
    plt.legend()
    plt.tight_layout()
    out_path = os.path.join(REPORT_DIR, "pr_hmda_lr_rf.png")
    plt.savefig(out_path, dpi=200)
    plt.close()
...
def main():
    ...
    hmda_probs, y_test_hmda = train_lr_rf_for_scheme("HMDA", full, train, valid, test)
    plot_roc_pr_hmda(hmda_probs, y_test_hmda)

    print("\n=== Training HMDA+ACS+PMMS scheme for RF importance ===")
    fusion_probs, y_test_fusion = train_lr_rf_for_scheme("HMDA+ACS+PMMS", full, train, valid, test)
    plot_rf_importance(fusion_probs, scheme_name="HMDA+ACS+PMMS", top_k=20)
